...
func NewWorker(ctx context.Context, config config.Config) (*Worker, error) {
	...
	exampleTasks := example.NewExampleTasks(observer, exampleRepository, config)
	...
	worker.Use(exampleMiddlewares.HandleTask)

	worker.Register(example.ExampleTasksMakeOnboarding, exampleTasks.MakeOnboarding)
	worker.Register(example.ExampleTasksReconcile, exampleTasks.Reconcile)

	/* SCHEDULEMENTS */

	worker.Schedule(example.ExampleTasksReconcile, nil, "* * * * *") // Every minute
			...
			go func() {
				...
				err := http.ListenAndServe(fmt.Sprintf(":%d", config.Worker.HealthPort),
					http.TimeoutHandler(http.HandlerFunc(healthEndpoints.GetWorkerHealth),
						config.Service.GracefulTimeout, kit.HTTPErrRequestTimeout.String()))
